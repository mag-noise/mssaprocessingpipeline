function import_data(data_location, debug, only_path)
    ...
    if(~only_path)
        ...
        if(~iscdf)
            ...
            data = cdfread(data_location);
            inboard = data(:,8);
            inboard = reshape(cell2mat(inboard), 3 , []);
            assignin('base', "inboard", inboard);
            outboard = data(:,10);
            outboard = reshape(cell2mat(outboard), 3 , []);
            assignin('base', "outboard", outboard);
            t_fit = cellfun(@todatenum, data(:, 3));
            assignin('base', "t_fit", t_fit);
